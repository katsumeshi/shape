...
class ScaleScreen extends React.Component {
	...
	public render() {
		...
		const date = type === "create" ? new Date() : this.props.navigation.getParam("date");
		const weight = type === "create" ? `${this.props.weight}` : this.props.navigation.getParam("weight");
		return (
			<React.Fragment>
				<Header
					leftComponent={
...
export default connect(state => ({ auth: state.auth, ...state.health.data[0] }))(ScaleScreen);
